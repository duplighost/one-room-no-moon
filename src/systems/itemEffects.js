  ...
  bloodTithe() {
    hooks.on('onKill', 'bloodTithe', () => {
      ...
      if (p._tithe >= need) {
        ...
        if (p.hp < p.maxHp) {
          p.hp += 1;
          addFloat(state.room, p.x, p.y - 44, 'TITHE +1', '#ff6b6b');
          sfx('care');
        }
      }
    });
      ...
      if (c.pounce > 0) {
        ...
        for (const e of room.enemies) {
          if (e.hp > 0 && dist(c.x, c.y, e.x, e.y) < 22 + e.r) {
            ...
            e.slowMul = 0.6;
            c.pounce = 0;
            addFloat(room, e.x, e.y - 30, 'GIGI', '#f9f6ee', false, 0.5);
            break;
          }
        }
      } else {
  ...
  sutureEngine() {
    hooks.on('onRoomClear', 'sutureEngine', () => {
      ...
      if (p.hp < p.maxHp && Math.random() < 0.25 + 0.18 * st('sutureEngine')) {
        p.hp += 1;
        addFloat(state.room, p.x, p.y - 44, 'SUTURE +1', '#b9ffb8');
      }
    });
  },
  riftCapacitor() {
    hooks.on('onRoomClear', 'riftCapacitor', () => {
      ...
      if (p._capacitor >= need) {
        p._capacitor = 0;
        if (p.hp < p.maxHp) { p.hp += 1; addFloat(state.room, p.x, p.y - 44, 'CAPACITOR +1', '#9ec6ff'); }
      }
    });
  },
  blackLotus() {
